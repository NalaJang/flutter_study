...
class HttpProvider with ChangeNotifier {
  ...
  Future<void> getPhotos() async {
    List<HttpModel>? data = await _fetchPost(pageNo: currentPage, pageLimit: 10);
    modelList = data;
    currentPage = 2;
  ...
  Future<List<HttpModel>> _fetchPost({required int pageNo, required int pageLimit}) async {
    try {
      // "http.get" : GET 방식의 Rest API 호출
      http.Response _response = await http.get(
        // "Uri.parse" : 문자열을 기반으로 URI 객체 생성
        Uri.parse("https://picsum.photos/v2/list?page=$pageNo&limit=$pageLimit")
      );
